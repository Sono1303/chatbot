...
def get_optimal_choice(sentence, intents_list):
	...
	if last_user_input is not None:
		if set(last_user_input) == set(extract_ingredients(sentence)) and list_optimal_choice[0]:	
		...
		for choice in intents_list:
			...
			count = sum(1 for ingredient in list_ingredient if extract_ingredients(ingredient)[0] in ingredients)
			if count == 0:
				continue
	...
	last_user_input = extract_ingredients(sentence)
...
def get_response(sentence):
	...
	if last_user_input != extract_ingredients(sentence):
